...
def run_inference(config):
    ...
    for batch_idx, start in enumerate(tqdm(range(0, len(ds), config.batch_size), total=n_batches, desc=config.run_name)):
        ...
        # progress + running WER/CER so we can spot if a run is going off the rails early.
        # batch 5/10/50 give early sanity checks; every 100 after that for ongoing tracking.
        if batch_idx in (5, 10, 50) or (batch_idx > 0 and batch_idx % 100 == 0):
            ...
            logger.info(
                "batch=%d samples_processed=%d running_WER=%.4f running_CER=%.4f",
                batch_idx, start + len(audio_arrays), running_wer, running_cer,
            )
